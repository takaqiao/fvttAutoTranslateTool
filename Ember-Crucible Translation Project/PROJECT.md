...
战役包 19234 / 19605 = **98%**，战役正文与收尾零碎均已完成。

**下一步是三块「覆盖率看不见」的欠账**，操作步骤见 `PARALLEL-RUNBOOK.md` 第 6.5 节：
第 8c 项剩余 26.0 万（补缺块）· 第 8j 项 4.8 万（译文里留着英文已删的内容，要**删**不要补）·
孪生包 `ember.adventure`（88% 可由 TM 脚本直填，独有部分仅 14.2 万）。

**翻译已改为并行推进**（阶段 20 起）：一轮 10–12 个单元、约 30 万英文字符，
译者自检闸门 + 对抗式审校 + 跨单元术语核对。做法与硬约束见阶段 20 日志，
**每次唤醒的操作手册是 `PARALLEL-RUNBOOK.md`**（自动循环也读它）。
五轮实测：约 7000 条 / **140 万英文字符**落盘**零拒绝**，标记漂移五轮**只降不升**
（LINK 689→624 / BLOCK 584→519 / INLINE 265→230 / TRUNCATED 69→51）。

剩余约 **50 万字符**：战役包常规待译 4.8 万 + 第 8c 项 26.0 万 + 第 8j 项 4.8 万 +
孪生包独有 14.2 万。
...
**孪生包填充脚本已就绪**（`tm/fill_twin.py`，尚未落盘）：crucible 侧到 98% 后 TM 覆盖率
88% → **98%**，12,560 条 / 约 817 万字符可脚本直填，闸门 `--dry` 实测 0 拒绝。
它不直接写 `compendium/cn`，只出 batch 交给 `apply_translations.py`，照样过三道闸。
**第 6 批已落盘，现在正是重跑它的时候**（被弃填的 554 条里 66% 就是本批修好的页）。
...
| 8o | └ **并行第 5 批**：战役收尾 ×3 + 补缺块 ×8 | 🔶 进行中 |
| 8p | └ **世界地图专名重定**：Break/Catch/Hoist/Wheel/Sail/Crown/Mordant/WINDBARE 等被当普通词译掉 | ⬜ 见阶段 23 |
| 8q | └ scene notes 键被上游改成随机 ID，298 条旧译成死键 | ✅ 第 5 批已按新键重新落 |
| 8j | └ **译文多出内容**（英文已删、中文还留着）：189 条 / 776 区块 / 约 4.8 万字符 | ⬜ `measure_stale_extra.py` 出清单 |
| 8k | └ **中文坐在错误活路径上**：2 处已确认（Spellbreaker `Storage`、Mythspire `Ancient Lift`） | 🔶 已退回待译，排下一轮 |
