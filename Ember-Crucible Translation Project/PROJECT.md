...
**下一步**：**冒烟验证** —— 清单在第 7 节末尾。唯一无法靠脚本证实的环节，至今没做过。
重点看日历那排 tooltip（本版修的就是它）。
...
>
> 发版前**必做**：`flatten_lang.py`（不加 `--write`）确认两个仓库都是
> `拍平前 == 拍平后 == 英文键数`。这一步就是为了挡住 1.1.0 那次 77% lang 失效。
...
`Bludgeoning`钝击 · `Fire`火焰 · `Corruption`腐化 · `Fortitude`坚韧防御 ·
...
# 2c. class 属性漂移（闸门的签名只取标签名，看不见 class）
#     section.block gamemaster / ul.complex-check / sup.system-swap-inline 都是功能性的
python "$P\3-常用脚本\qa\scan_class_drift.py"           --repo <repo> --out <reportDir>\class_drift.json
...
| 6b | └ 清理孤儿译文（babele 匹配不到 key 的死文本） | ✅ 2026-08-12 `prune_dead.py` 清 crucible 74 / ember 1435，省 480 KB，有效译文键数不变 |
| 6c | └ 发 crucible-cn | ✅ 0.9.0（2026-08-10）、0.9.1（2026-08-12） |
| 5b | 修复已发布译文的格式类缺陷 | ✅ 2026-08-12 随 E 项一并清完 |
| 5c | 裁决剩余真实术语分歧 | ✅ 2026-08-12 全部裁完，`glossary_ec.disputes.json` 已转为**裁决记录**（每条附依据） |
...
| 5g | 补译被整段丢掉的规则正文（TRUNCATED） | ✅ 23 条已补译，两个仓库复测均为 0 |
| 5h | 段落结构与加粗漂移清扫（BLOCK / INLINE） | ✅ 阶段 26/27 归零（BLOCK 584→0、INLINE 265→0） |
| 9 | **真实 Foundry 世界冒烟验证**（管线改造后必做） | ⬜ **唯一还没做的事**（脚本证不了，需要开世界看） |
...
| 8i | └ **并行第 2 批**：Winding Trail / GM Guide / Expedition / Ch2 Events / Players' Guide / Mythspire + 随机表 ×3 | ✅ 722 条 / 27.4 万字符 |
| 8l | └ **并行第 3 批**：Disturbed Earth ×8 / Arctus Plateau Gazetteer ×3 / 小卷合集 | ✅ 449 条 / 30.8 万字符 |
| 8m | └ **并行第 4 批**：actors 桶 ×11（内嵌物品与能力） | ✅ 3106 条 / 33 万字符，覆盖 89% |
...
| 8p | └ **世界地图专名重定** | ✅ 2026-08-12，两包各 14 个针脚。地形通名（`Mounts`→山脉 而非坐骑、`Range`→山脉 而非射程）、有正文锚点的地名（WINDBARE→温赤荒、KAUSTIC HINTERLANDS→考斯提克边地、CASCAAL GOLDS→卡斯卡尔金原）、动词/形容词当地名（Sail 航行→帆、Hoist 升起→吊索、Break 破坏→断口、Catch 接住→渔获、Mordant 腐蚀性的→莫丹特）、张冠李戴（`Elvan` 精灵语→埃尔凡，这是聚落不是语言；`Elwin's Seat` 座位→之座）|
