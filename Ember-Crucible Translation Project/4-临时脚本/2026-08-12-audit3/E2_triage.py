...
def main():
    ...
    for i, it in enumerate(items, a.start):
        ...
        # 多重集减法在「同一标记出现多次、只是次数减少」时会误报。
        # 真正硬的信号是**绝迹/全新**：新英文里次数归零、或旧英文里根本没有。
        only_old = Counter({k: v for k, v in (to - tn).items() if tn[k] == 0})
        only_new = Counter({k: v for k, v in (tn - to).items() if to[k] == 0})
        cn_has_old = only_old & tc  # 中文里还留着已绝迹的旧标记
        cn_has_new = only_new & tc  # 中文里已有全新标记
        cn_miss_new = Counter({k: v for k, v in only_new.items() if tc[k] == 0})
        verdict = []
        if cn_has_old:
