...
function extractDocument(doc, documentType, mappings) {
  ...
  for (const [field, spec] of Object.entries(mapping)) {
    ...
    switch (spec.converter) {
      ...
      case 'emberEncounterTokens': {
        // Two levels of unkeyed arrays (spawn groups -> actor entries), keyed by
        // the English override name — same shape as `nameCollection`. The
        // translate direction lives in `release/runtime-converters.js`.
        const map = {};
        for (const group of toArray(value)) {
          for (const entry of toArray(group?.actors)) {
            const n = entry?.tokenData?.name;
