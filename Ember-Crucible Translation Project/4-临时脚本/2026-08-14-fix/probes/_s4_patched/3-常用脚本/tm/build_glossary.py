...
def harvest(en_dir, cn_dir, label, pairs, pending):
    ...
    for fn in sorted(f for f in os.listdir(en_dir) if f.endswith('.json') and not f.startswith('_')):
        ...
        walk_pairs(en_folders, en_folders, cand)
        for s, _ in cand:
            if is_term(s):
        ...
        walk_pairs(en_folders, cn_doc.get('folders', {}), got)
...
def main():
    ...
    # ⚠ 这个分类**没有角色维度**，而「双语后缀加不加」在本项目里正是角色约定：
    # `name` 用双语并列「护盾术 Shield」，`tokenName` / `adjective` / `levels` /
    # `folders` / 表结果行用裸中文。所以「剥掉后缀后中文相同」恰恰是**约定正确**
    # 的标志，不是不一致的标志。2026-08-14 实测：该桶 392 条里，39 条 adjective、
    # 250 条 tokenName、24 条 folders 都在其中，照「可批量归一」做就是 392 处回归
    # （其中 39 条 adjective 会让 crucible item-physical.mjs:324 在运行时拼出
    # 「决心 Determination护符」），而归一后英文在位、中文含中文、markup 签名不变，
    # apply_translations.py 的三道闸全过、既有扫描一条也看不见。
    # 在把角色（叶子路径的最后一段）带进 pairs 之前，这个桶只能当**待人工分辨**
    # 的线索，绝不能当批量任务。
    ...
    for en_s, cands in conflicts.items():
        ...
        if len({bare(c, en_s) for c in cands}) == 1:
            fmt_inconsistent[en_s] = cands   # 多数是角色约定差异，少数才是真漏加后缀
        else:
            term_inconsistent[en_s] = cands  # genuinely different Chinese
    print(f'shipped: 同名两写法（多数是角色约定，勿批量归一）: {len(fmt_inconsistent)}')
    ...
    dump(os.path.join(OUT, 'glossary_ec.disputes.json'), {
        ...
        'shippedBilingualFormatInconsistency': {
            '_note': '同一专名的中文相同，只是双语后缀（" English"）时加时不加。'
                     '⚠ **不得批量归一。** 本项目里「加不加后缀」是字段角色的约定：'
                     '`name` 双语并列「护盾术 Shield」，`tokenName` / `adjective` / '
                     '`levels` / `folders` / 表结果行裸中文。这个桶没有角色维度，'
                     '分不出「同一角色内真漏加了后缀」与「本来就分属两个角色」——'
                     '2026-08-14 实测 392 条里 39 条 adjective、250 条 tokenName、'
                     '24 条 folders 属于后者，归一即 392 处回归（adjective 会被 '
                     'crucible item-physical.mjs:324 拼进物品名，玩家看到'
                     '「决心 Determination护符」）。逐条按所在字段人工判读。',
