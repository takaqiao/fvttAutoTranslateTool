...
* **Same English, different Chinese.** A naive `dict[en] = cn` takes whichever
  ...
  Keying on the LAST SEGMENT is not enough: it collapses all three `.name`
  populations back together, which is the very mistake stage 23 warned about.
  Switching from last-segment to structural path cut ambiguous keys from 697 to
  530. Keying on the ENGLISH ALONE is worse still -- it has no role dimension at
  all, so `name`'s bilingual "护盾术 Shield" gets poured into the bare-Chinese
  slots of `tokenName` / `adjective`. That fallback was removed 2026-08-14; a
  miss now goes to the translator, and what the role-blind key would have
  suggested is recorded in the report instead of being written. Where a key
  still has competing Chinese, the majority wins and the conflict is reported
...
STRUCT = {
    ...
    # 2026-08-14 补：这些角色/容器段此前不在表内，于是在 shape 键里被抹成空。
    # `tokenName` 是裸中文约定、顶层 `name` 是双语并列约定，两者都缺就会互相污染；
    # `regions.name` 同理（裸中文）。数量按两仓 en 全量普查。
    'tokenName', 'adjective', 'levels', 'regions', 'categories', 'behaviors',
    'tokens', 'navName', 'condition', 'appearance', 'contentOverview',
    'contentGamemaster',
}
...
def shape_of(path):
    ...
    Additionally the LAST segment is always kept: it is the field role, and a
    whitelist can never keep up with roles the upstream adds. Whatever the
    whitelist misses loses its role dimension entirely, which is how two
    mutually exclusive conventions (bilingual "护盾术 Shield" for .name vs bare
    Chinese for .tokenName / .adjective) end up in one bucket. Keeping the last
    segment can only make a key too narrow (fewer hits, more human work); it can
    never attribute one role's convention to another.
    """
    parts = path.split('.')
...
def main():
    ...
    for path, e, c in dst:
        ...
        key = e.strip()
        shape = shape_of(path)
        hit = shaped.get((shape, key))
        if not hit:
            # 曾经退化成 `plain.get(key)`：纯英文键、零角色的全库多数派。见文件头。
            blind = plain.get(key)
            if blind:
                stat['roleblind_withheld'] += 1
                if len(roleblind) < 500:
                    roleblind.append({'path': path, 'en': key[:120],
    ...
    print(f'  TM 命中并填充   {stat["filled"]:>7}   （全部走形状键）')
    print(f'  仅无角色键有候选，已拦下 {stat["roleblind_withheld"]:>7}   ← 见报告 roleblind_withheld')
