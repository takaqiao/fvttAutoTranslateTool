...
这条踩过一次，代价是差点把「一条没做」误判成「已完成」。

`apply_translations.py` 比的是**标记多重集**。所以它能当完成信号，**前提是这批缺陷
定义在标记上**：

| 批次 | 缺陷定义在 | 闸门能否当完成信号 |
|---|---|---|
| 第 6 批 realign（补缺块/删多余块） | 块数 → 标记数 | ✅ 能。补齐前必然被拒 |
| 第 7 批 sigfix（签名失配） | 标记本身 | ✅ 能。签名相等就是缺陷定义 |
| **追平批（英文变过 / 数字没覆盖）** | **内容** | ❌ **不能**。这些条目的标记本来就是对的，闸门在动手前后**都报 0 拒绝** |

追平批第一次发出去时，13 个 agent 全被额度杀死、一条没写，而闸门报
`applied 378 / rejected 0` —— 看着像做完了。**是 `collect_realign.py` 的 `UNTOUCHED 378`
才戳穿的。**

所以：**发批次前先想清楚「这批的完成度靠什么判」**，并把它写进提示词的交付标准。
追平批用 `parallel/check_catchup.py`（查英文数字是否都在中文里 + 长度比 ≥ 0.25）。

> 更根本的一条：**机械检查只能证伪，不能证明。** `check_catchup` 通过只说明数字和篇幅
> 达标，说明不了叙述、条件、因果有没有跟上 —— 那必须逐句对照英文。所以「英文变过」
> 类条目即使机械检查全绿，也仍要读。
...
| dnd5e 孪生包 `actors` 桶独有 | ⬜ 1,019 条 / 11.9 万字符 —— 真正需要翻的那部分，形态见下 |

> **孪生包 actors 桶是什么**（切批前先读，能省很多事）：1,019 条里 **729 条是 `.name`**，
> 而且绝大多数是 **D&D 5e 的标准法术与特性**（Longstrider / Barkskin / Darkvision /
> Gaseous Form / Stinking Cloud / Dominate Beast …），**不是 Ember 自有世界观内容**。
> 所以这批的原则是**沿用国内通行的 5e 译名，不要另造**。
>
> 现成对照源（覆盖率实测）：`glossary_ec.json` 命中 **203/729**、
> PF2E 主表 `fvtt\glossary.json` 命中 **63/729**（大步奔行 / 树肤术 / 气化形体 / 臭云术 等，
> 译法可用）。两者合计约三分之一。
> ⚠ PF2 主表按第 8 节既有决议**只作低优先级建议源，命中必须人工确认**（世界观不同，
> `Restrained` 本项目作「受缚」、PF2 作「受制」就是例子）。
>
> **本地没有权威的 dnd5e 合集中文**：`5e_chn` 模块只有 1367 个 UI 键（i18n），
> 不含法术名；所以 `resolve_generic_fallback.py` 把它们留作残余是对的，不是漏算。
