...
test('native Stride binds the plan before starting and waits for its original finished promise',async()=>{
 ...
 const user={id:'owner'},game={user},calls=[],plan={id:'p'},token={id:'t',uuid:'Scene.s.Token.t',x:0,y:0,elevation:0,parent:{tokens:new Map()},object:{async planMovement(options){calls.push(['plan',options]);return plan}},movement:{id:'p',state:'planned',user,finished:Promise.resolve(true)},async startMovement(id){calls.push(['start',id]);this.movement.state='completed';return true}};token.parent.tokens.set('t',token);
