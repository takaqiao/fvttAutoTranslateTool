...
test('native Poison and First Aid receive exact actor/target, continuation and player variant',async()=>{
 const calls=[],actor={id:'healer',uuid:'Actor.healer'},patient={uuid:'Actor.patient'},target={uuid:'Scene.s.Token.t',actor:patient},healer={uuid:'Scene.s.Token.h',actor,object:{id:'h'}},user={id:'owner'},continuation={actorUuid:actor.uuid,cardId:'card',nonce:'nonce'};
 const game={user,messages:new Map(),pf2e:{actions:new Map(['treat-poison','administer-first-aid'].map(key=>[key,{use:async args=>{calls.push([key,args]);const roll={total:25,options:{degreeOfSuccess:2}},message={id:key,author:user,speaker:{actor:actor.id},rolls:[roll],flags:{pf2e:{context:{type:'skill-check',outcome:'success',options:args.rollOptions,target:{actor:patient.uuid,token:target.uuid}}}},async update(changes){this.flags[M]={medicNative:changes[`flags.${M}.medicNative`]};}};game.messages.set(message.id,message);return [{actor,roll,message}];}}]))}};
 const delegate=createMedicNative({game,choose:async()=> 'stabilize'});
 for(const branch of ['treat-poison','administer-first-aid']){const result=await delegate({actor,target,healer,user,branch,continuation,validate:()=>{}});assert.equal(result.checkId,branch);assert.equal(game.messages.get(branch).flags[M].medicNative.targetUuid,'Scene.s.Token.t');}
...
test('installed unchanged Workbench Assurance binds its actual roll card before the treatment result',{skip:!existsSync(wbPath)},async()=>{
 ...
 const form={find(selector){const node=nodes[selector.match(/name="([^"]+)"/)?.[1]];return {0:node,length:node?1:0,val(value){if(value===undefined)return node?.value;if(node)node.value=value;return this;},prop(){return this;},trigger(){return this;}};}};
 await f.observed.dialog.options.buttons.yes.callback(form);const result=await f.operation;assert.equal(result.status,'delegated');assert.equal(result.checkId,f.cards[0].id);assert.equal(result.resultId,f.cards[1].id);assert.equal(f.cards.length,2);assert.equal(f.cards[0].roll[0].total,22);assert.equal(f.cards[0].flags[M].medicWorkbench.checkKind,'assurance');assert.equal(f.cards[1].flags[M].medicWorkbench.checkId,f.cards[0].id);assert.equal(f.observed.check,undefined);
});
...
test('Workbench healing cards keep the original patient in Toolbelt after the owner retargets',async()=>{const f=detachedWorkbench();await flush();await f.observed.dialog.options.buttons.yes.callback(html);f.game.user.targets=new Set([{id:'different',document:{uuid:'Scene.s.Token.different'}}]);f.release();await flush();await f.observed.healReady();await f.operation;assert.deepEqual(f.cards[0].flags['pf2e-toolbelt']?.targetHelper?.targets,['Scene.s.Token.t']);assert.equal(f.cards[0].flags.treat_wounds_battle_medicine.id,'t');});
...
test('Workbench validates immediately before native submission and propagates cancellation',async()=>{
 const actor={},healer={object:{actor}},target={object:{actor:{}}},game={user:{id:'owner',targets:new Set()},modules:new Map([['xdy-pf2e-workbench',{active:true}]]),packs:new Map()},calls=[];
 ...
 const result=await createMedicNative({game,canvas:{tokens:{}},Dialog})({actor,healer,target,user:game.user,branch:'battle-medicine',validate:()=>{}});
