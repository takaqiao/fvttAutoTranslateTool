...
test('worn tools with a free hand permit a source-based check without a spatial measurement',()=>assert.equal(validateTreatment({actor:actor(),condition:condition(),facts:{dc:22,restricted:false,continuous:false}}).dc,22));
for(const distance of [60,null,undefined])test(`GM-adjudicated distance ${distance} does not replace the real source DC`,()=>assert.equal(validateTreatment({actor:actor(),condition:condition(),distance,facts:{dc:22,restricted:false,continuous:false}}).dc,22));
for(const [name,change] of [
 ['wrong condition',x=>x.condition.slug='frightened'],['no tools',x=>x.actor.items=[]],['occupied hands',x=>x.actor.handsFree=0],['missing DC',x=>delete x.facts.dc],['unknown circumstances',x=>delete x.facts.continuous],['artifact without Legendary Medic',x=>x.facts.restricted=true],['continuous source',x=>x.facts.continuous=true],['granted condition',x=>x.condition.flags={pf2e:{grantedBy:{id:'parent'}}}]
