...
function fixture({kind='strike',map=0,outcomes=['success','success'],save='failure',spellSave=false,basic=true,attack=!spellSave,targets=kind==='swipe'?2:1,multi=false,charged=true,overlays=false,consumeFails=false}={}){
 ...
 const targetDocs=Array.from({length:targets},(_,i)=>({id:`target${i}`,uuid:`Scene.s.Token.target${i}`,documentName:'Token',parent:{id:'s'},actor:{uuid:`Actor.target${i}`,type:'npc'},object:{distanceTo:()=>5}}));
 ...
 const origin={id:'pcToken',uuid:'Scene.s.Token.pcToken',parent:{id:'s'},actor,object:{distanceTo:()=>5,checkCollision:()=>false}};actor.getActiveTokens=()=>[origin];actor.getReach=()=>5;
...
test('Spell Swipe rejects nonadjacent targets before payment',()=>setup({kind:'swipe'},async f=>{f.targetDocs[0].object.distanceTo=()=>10;await assert.rejects(f.use,/相邻/);assert.equal(f.calls.length,0)}));
...
test('source token and native reach must be known for all selected attacks',()=>setup({},async f=>{f.origin.object.distanceTo=()=>10;await assert.rejects(f.use,/触及/);assert.equal(f.calls.length,0)}));
...
test('each Spell Swipe Strike revalidates reach after payment and the previous Strike',()=>setup({kind:'swipe',multi:true},async f=>{const original=f.actor.system.actions[0].variants[0].roll;f.actor.system.actions[0].variants[0].roll=async opts=>{const r=await original(opts);f.origin.object.distanceTo=()=>20;return r};await assert.rejects(f.use,/触及/);assert.equal(f.calls.filter(c=>c.kind==='attack').length,1);assert.equal(f.calls.filter(c=>c.kind==='payment').length,1);assert.equal(f.message.flags[NS].spellCombinationUse.state,'error')}));
